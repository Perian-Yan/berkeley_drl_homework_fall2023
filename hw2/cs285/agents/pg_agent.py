...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        # beyond this point.
        # NOTE: args.batch_size is the number of transitions sampled using utils.sample_trajectories
        # For obs: Sequence[np.ndarray], every ndarray has a shape of (T,dim_obs). batch_size is the sum of T for all trajs.
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            q_values = [self._discounted_return(reward_one_traj) for reward_one_traj in rewards]

        else:
            # Case 2: in reward-to-go PG, we only use the rewards after timestep t to estimate the Q-value for (s_t, a_t).
            # In other words: Q(s_t, a_t) = sum_{t'=t}^T gamma^(t'-t) * r_{t'}
            # TODO: use the helper function self._discounted_reward_to_go to calculate the Q-values
            q_values = [self._discounted_reward_to_go(reward_one_traj) for reward_one_traj in rewards]
